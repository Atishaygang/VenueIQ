  ...
  return (
    <>
      <button 
        ...
        className={`fixed bottom-20 right-4 z-40 bg-gradient-to-r from-[#6c63ff] to-[#4b45bd] p-4 rounded-full shadow-[0_0_20px_rgba(108,99,255,0.4)] text-white hover:scale-105 transition-transform ${isListening ? 'animate-pulse ring-4 ring-[#6c63ff]/50' : ''}`}
      >
        <Mic size={24} />
      </button>

      {isOpen && (
        <div className="fixed inset-x-0 bottom-16 z-50 bg-[#13131a] border-t border-gray-800 rounded-t-3xl shadow-2xl h-[350px] max-h-[60vh] flex flex-col transform transition-transform md:max-w-md md:mx-auto">
          {/* Header */}
          ...
          <div className="p-3 border-t border-gray-800 bg-[#0a0a0f]">
            ...
            <div className="flex space-x-2">
              <form onSubmit={handleTextSubmit} className="flex-1 flex bg-[#1a1a24] border border-gray-700 rounded-full px-4 py-2">
                ...
                />
                <button type="submit" className="text-[#6c63ff] ml-2"><Send size={16}/></button>
              </form>
              <button 
                onClick={handleMicClick}
                className={`p-3 rounded-full bg-[#6c63ff] text-white transition-all ${isListening ? 'animate-pulse shadow-[0_0_15px_rgba(108,99,255,0.6)]' : ''}`}
              >
                <Mic size={18} />
              </button>
            </div>
